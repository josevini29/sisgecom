...
public class FrmCadFornecedorFXML implements Initializable {

    @FXML
    AnchorPane painel;
    @FXML
    TextField cdFornecedor;
    @FXML
    TextField dsFornecedor;
    @FXML
    CheckBox inAtivo;
    @FXML
    TextField dsEmail;
    ...
    ArrayList<ContasHit> listConta = new ArrayList<>();
    ...
    public void pesquisarFornecedor() {
        Tela tela = new Tela();
        String valor = tela.abrirListaPessoa(new Fornecedor(), false);
        if (valor != null) {
            cdFornecedor.setText(valor);
            validaCodigoFornecedor();
        }
    }

    @Override
    public void initialize(URL url, ResourceBundle rb) {
        FuncaoCampo.mascaraNumeroInteiro(cdFornecedor);
        ...
        FuncaoCampo.mascaraTexto(dsObs, 500);
        FuncaoCampo.mascaraTexto(dsFornecedor, 200);
        ...
        cdFornecedor.focusedProperty().addListener((ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
            if (!newPropertyValue) {
                validaCodigoFornecedor();
            }
        });
    ...
    private void validaCodigoFornecedor() {
        if (!cdFornecedor.getText().equals("") & fornecedor == null) {
            try {
                fornecedor = new Fornecedor();
                fornecedor.setCdFornecedor(Integer.parseInt(cdFornecedor.getText()));
                dao.get(fornecedor);
                inAtivo.setSelected(fornecedor.getInAtivo());
                dsObs.setText(fornecedor.getDsObs());
                lblCadastro.setText(Numero.getCadastro(fornecedor.getCdUserCad(), fornecedor.getDtCadastro()));

                pessoa = new Pessoa();
                pessoa.setCdPessoa(fornecedor.getCdPessoa());
                dao.get(pessoa);
                dsFornecedor.setText(pessoa.getDsPessoa());
                ...
                cdFornecedor.setEditable(false);
            } catch (Exception ex) {
                Alerta.AlertaError("Notificação", ex.getMessage());
                fornecedor = null;
                cdFornecedor.requestFocus();
                return;
            }
            atualizaEnderecoHit();
            atualizaTelefoneHit();
        ...
        if (!nrCpf.getText().equals("")) {
            ...
            try {
                ...
                for (Object obj : pessoaProc) {
                    ...
                    String where = fornecedor != null ? " WHERE $cdPessoa$ = " + pessoaObj.getCdPessoa() + " AND $cdFornecedor$ != " + fornecedor.getCdFornecedor()
                            : " WHERE $cdPessoa$ = " + pessoaObj.getCdPessoa();
                    long qtFornecedor = dao.getCountWhere(new Fornecedor(), where);
                    if (qtFornecedor > 0) {
                        Alerta.AlertaError("CPF inválido", "Já existe Fornecedor com este CPF cadastrado.");
                        nrCpf.requestFocus();
                        return;
                    }
                    pessoa = pessoaObj;
        ...
        if (!nrCnpj.getText().equals("")) {
            ...
            try {
                ...
                for (Object obj : pessoaProc) {
                    ...
                    long qtFornecedor = dao.getCountWhere(new Fornecedor(), where);
                    if (qtFornecedor > 0) {
                        Alerta.AlertaError("CNPJ inválido", "Já existe Fornecedor com este CNPJ cadastrado.");
                        nrCnpj.requestFocus();
                        return;
                    }
                    pessoa = pessoaObj;
    ...
    @FXML
    public void salvar() {
        if (dsFornecedor.getText().equals("")) {
            Alerta.AlertaError("Campo inválido", "Campo nome do fornecedor é obrigatório.");
            return;
        }

        if (!nrCpf.getText().equals("")) {
            if (nrRg.getText().equals("")) {
                Alerta.AlertaError("Campo inválido", "Campo RG do fornecedor é obrigatório.");
                return;
            }

            if (dtNasc.getText().equals("")) {
                Alerta.AlertaError("Campo inválido", "Campo Dt. Nascimento do fornecedor é obrigatório.");
                return;
            }

            if (TrataCombo.getValueComboSexo(tpSexo) == null) {
                Alerta.AlertaError("Campo inválido", "Campo Sexo do fornecedor é obrigatório.");
                return;
            }

            if (TrataCombo.getValueComboSexo(tpCivil) == null) {
                Alerta.AlertaError("Campo inválido", "Campo Estado Civil do fornecedor é obrigatório.");
                return;
            }
        } else if ((!nrCnpj.getText().equals(""))) {
            if (dsApelido.getText().equals("")) {
                Alerta.AlertaError("Campo inválido", "Campo Razão Social do fornecedor é obrigatório.");
                return;
            }
        } else {
        ...
        for (ContasHit con : listConta) {
            if (!con.codBanco.getText().equals("") || !con.nrAgencia.getText().equals("") || !con.nrConta.getText().equals("")) {
                if (con.codBanco.getText().equals("")) {
                    Alerta.AlertaError("Campo inválido!", "Campo Código do Banco é obrigatório.");
                    return;
                }
                if (con.nrAgencia.getText().equals("")) {
                    Alerta.AlertaError("Campo inválido!", "Campo Nª da Agência é obrigatório.");
                    return;
                }
                if (con.nrConta.getText().equals("")) {
                    Alerta.AlertaError("Campo inválido!", "Campo Nª da Conta é obrigatório.");
                    return;
                }
            }
        }
        ...
        pessoa.setDsPessoa(dsFornecedor.getText());
        ...
        try {
            ...
            if (fornecedor == null) {
                ...
                fornecedor.setDsObs(dsObs.getText());
                fornecedor.setDtCadastro(Data.getAgora());
                fornecedor.setCdUserCad(FrmMenuFXML.usuarioAtivo);
                fornecedor.setInAtivo(inAtivo.isSelected());
                dao.save(fornecedor);
            } else {
                fornecedor.setCdPessoa(pessoa.getCdPessoa());
                fornecedor.setDsObs(dsObs.getText());
            ...
            for (ContasHit con : listConta) {
                if (!con.codBanco.getText().equals("")) {
                    if (con.isExcluir) {
                        if (con.cdFornecedor != null && con.cdConta != null) {
                            Conta conta = new Conta();
                            conta.setCdFornecedor(con.cdFornecedor);
                            conta.setCdConta(con.cdConta);
                            dao.delete(conta);
                        }
                    } else {
                        if (con.cdFornecedor != null && con.cdConta != null) {
                            ...
                            conta.setCdFornecedor(con.cdFornecedor);
                            conta.setCdConta(con.cdConta);
                            conta.setCdBanco(Integer.parseInt(con.codBanco.getText()));
                            conta.setNrAgencia(con.nrAgencia.getText());
                            conta.setNrConta(con.nrConta.getText());
                            dao.update(conta);
                        } else {
                            Conta conta = new Conta();
                            conta.setCdFornecedor(fornecedor.getCdFornecedor());
                            conta.setCdBanco(Integer.parseInt(con.codBanco.getText()));
                            conta.setNrAgencia(con.nrAgencia.getText());
                            conta.setNrConta(con.nrConta.getText());
                            dao.save(conta);
            ...
            dao.commit();
            Integer cod = fornecedor.getCdFornecedor();
            limpar();
            cdFornecedor.setText(cod.toString());
            validaCodigoFornecedor();
        } catch (Exception ex) {
            Alerta.AlertaError("Erro!", ex.getMessage());
            dao.rollback();
        ...
        Alerta.AlertaInfo("Concluído", "Fornecedor salvo com sucesso!");
    }
    ...
    //Final codigo telefone

    //Inicio codigo contas bancarias
    public void atualizaContaHit() {
        try {
            ArrayList<Object> contas = new ArrayList<>();
            if (fornecedor != null) {
                contas = dao.getAllWhere(new Conta(), "WHERE $cdFornecedor$ = " + fornecedor.getCdFornecedor() + " ORDER BY $cdConta$ ASC");
            }
            listConta.clear();
            if (contas.isEmpty()) {
                listConta.add(getContaNovo());
            }
            for (Object obj : contas) {
                Conta conta = (Conta) obj;
                TextField codBanco = new TextField();
                TextField dsBanco = new TextField();
                TextField nrAgencia = new TextField();
                TextField nrConta = new TextField();
                codBanco.setText(conta.getCdBanco().toString());
                nrAgencia.setText(conta.getNrAgencia());
                nrConta.setText(conta.getNrConta());

                Banco banco = new Banco();
                banco.setCdBanco(conta.getCdBanco());
                dao.get(banco);
                dsBanco.setText(banco.getDsBanco());

                ContasHit contasHit = new ContasHit();
                contasHit.cdConta = conta.getCdConta();
                contasHit.cdFornecedor = conta.getCdFornecedor();
                contasHit.codBanco = codBanco;
                contasHit.dsBanco = dsBanco;
                contasHit.nrAgencia = nrAgencia;
                contasHit.nrConta = nrConta;

                listConta.add(contasHit);
            }
        } catch (Exception ex) {
            Alerta.AlertaError("Erro!", "Erro ao iniciar tela.\n" + ex.toString());
        }
    ...
    public void atualizaListaConta() {
        int total = 0;
        for (ContasHit b : listConta) {
            if (!b.isExcluir) {
                total++;
            }
        }

        LayoutYConta = this.codBanco.getLayoutY();
        painelConta.getChildren().clear();
        Iterator it = listConta.iterator();
        for (int i = 0; it.hasNext(); i++) {
            ContasHit b = (ContasHit) it.next();
            if (!b.isExcluir) {
                b.codBanco.setPrefHeight(this.codBanco.getHeight());
                b.codBanco.setPrefWidth(this.codBanco.getWidth());
                b.codBanco.setLayoutX(this.codBanco.getLayoutX());
                b.codBanco.setLayoutY(LayoutYConta);
                b.codBanco.getStyleClass().addAll(this.codBanco.getStyleClass());
                b.dsBanco.setPrefHeight(this.dsBanco.getHeight());
                b.dsBanco.setPrefWidth(this.dsBanco.getWidth());
                b.dsBanco.setLayoutX(this.dsBanco.getLayoutX());
                b.dsBanco.setLayoutY(LayoutYConta);
                b.dsBanco.setEditable(this.dsBanco.isEditable());
                b.nrAgencia.setPrefHeight(this.nrAgencia.getHeight());
                b.nrAgencia.setPrefWidth(this.nrAgencia.getWidth());
                b.nrAgencia.setLayoutX(this.nrAgencia.getLayoutX());
                b.nrAgencia.setLayoutY(LayoutYConta);
                b.nrConta.setPrefHeight(this.nrConta.getHeight());
                b.nrConta.setPrefWidth(this.nrConta.getWidth());
                b.nrConta.setLayoutX(this.nrConta.getLayoutX());
                b.nrConta.setLayoutY(LayoutYConta);
                b.btnAddCon.setPrefHeight(btnAddCon.getHeight());
                b.btnAddCon.setPrefWidth(btnAddCon.getWidth());
                b.btnAddCon.setLayoutX(btnAddCon.getLayoutX());
                b.btnAddCon.setLayoutY(LayoutYConta);
                IconButtonHit.setIcon(b.btnAddCon, IconButtonHit.ICON_ADD);
                b.btnRemCon.setPrefHeight(btnRemCon.getHeight());
                b.btnRemCon.setPrefWidth(btnRemCon.getWidth());
                b.btnRemCon.setLayoutX(btnRemCon.getLayoutX());
                b.btnRemCon.setLayoutY(LayoutYConta);
                IconButtonHit.setIcon(b.btnRemCon, IconButtonHit.ICON_EXCLUIR);
                b.btnPesqBanco.setPrefHeight(btnPesqBanco.getHeight());
                b.btnPesqBanco.setPrefWidth(btnPesqBanco.getWidth());
                b.btnPesqBanco.setLayoutX(btnPesqBanco.getLayoutX());
                b.btnPesqBanco.setLayoutY(LayoutYConta);
                IconButtonHit.setIcon(b.btnPesqBanco, IconButtonHit.ICON_PESQUISA);
                painelConta.getChildren().add(b.codBanco);
                painelConta.getChildren().add(b.btnPesqBanco);
                painelConta.getChildren().add(b.dsBanco);
                painelConta.getChildren().add(b.nrAgencia);
                painelConta.getChildren().add(b.nrConta);
                painelConta.getChildren().add(b.btnAddCon);
                painelConta.getChildren().add(b.btnRemCon);
                LayoutYConta += (this.nrConta.getHeight() + 5);
            }
            addValidacaoConta(b, i, total);
        }

        painelConta.setPrefHeight(LayoutYConta
                + 10);
    }
    ...
    public void addValidacaoConta(ContasHit conta, int posicao, int total) {
        ...
        conta.btnPesqBanco.setOnAction((ActionEvent event) -> {
            Tela tela = new Tela();
            String valor = tela.abrirListaGenerica(new Banco(), "cdBanco", "dsBanco", null, "Lista de Bancos");
            if (valor != null) {
                conta.codBanco.setText(valor);
                validaCodigoBanco(conta.codBanco, conta.dsBanco);
            }
        });

        conta.codBanco.focusedProperty().addListener((ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue) -> {
            if (!newPropertyValue) {
                validaCodigoBanco(conta.codBanco, conta.dsBanco);
            }
        });

        conta.btnAddCon.setOnAction((ActionEvent event) -> {

            listConta.add(posicao + 1, getContaNovo());
            atualizaListaConta();
        });

        conta.btnRemCon.setOnAction((ActionEvent event) -> {
            if (total == 1) {
                listConta.add(getContaNovo());
            }
            listConta.get(posicao).isExcluir = true;
            atualizaListaConta();
        });

    }

    private void validaCodigoBanco(TextField codBanco, TextField dsBanco) {
        if (!codBanco.getText().equals("")) {
            try {
                Banco banco = new Banco();
                banco.setCdBanco(Integer.parseInt(codBanco.getText()));
                dao.get(banco);
                dsBanco.setText(banco.getDsBanco());
            } catch (Exception ex) {
                Alerta.AlertaError("Notificação", ex.getMessage());
    ...
    private ContasHit getContaNovo() {
        TextField codBanco = new TextField();
        TextField dsBanco = new TextField();
        TextField nrAgencia = new TextField();
        TextField nrConta = new TextField();
    ...
    public class ContasHit {

        public Integer cdConta;
        public Integer cdFornecedor;
        public TextField codBanco;
        public TextField dsBanco;
        public Button btnPesqBanco;
        public TextField nrAgencia;
        public TextField nrConta;
        public Button btnAddCon;
        public Button btnRemCon;
        public boolean isExcluir = false;

        public ContasHit() {
            this.btnPesqBanco = new Button();
            this.btnAddCon = new Button();
            this.btnRemCon = new Button();
        }
    }
    //Fim codigo conts bancarias
